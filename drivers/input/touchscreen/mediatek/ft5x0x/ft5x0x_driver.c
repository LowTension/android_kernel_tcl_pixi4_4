...
#define FT_I2C_ADDRESS (0x70)

static unsigned short force[] = { 0, FT_I2C_ADDRESS, I2C_CLIENT_END, I2C_CLIENT_END };
...
#if 0
#define FT5X0X_GPIO_AS_INT(pin) tpd_gpio_as_int(pin)
#define FT5X0X_GPIO_OUTPUT(pin, level) tpd_gpio_output(pin, level)
#else
...
#define ERROR_MEM      -3
#define ERROR_IIC      -2
#define FT5X0X_ADDR_LENGTH             2
#define IIC_MAX_TRANSFER_SIZE         8
#define IIC_DMA_MAX_TRANSFER_SIZE     250
#define I2C_MASTER_CLOCK              300


static s32 _do_i2c_read(struct i2c_msg *msgs, u16 addr, u8 *buffer, s32 len)
{
	s32 ret = -1;
	s32 pos = 0;
	s32 data_length = len;
	...
	data =
	    kmalloc(IIC_MAX_TRANSFER_SIZE <
			   (len + FT5X0X_ADDR_LENGTH) ? IIC_MAX_TRANSFER_SIZE : (len + FT5X0X_ADDR_LENGTH), GFP_KERNEL);
	if (data == NULL)
		return ERROR_MEM;
	msgs[1].buf = data;
	...
	while (pos != data_length) {
		...
		if (ret != 2) {
			TPD_DMESG("I2c Transfer error! (%d)", ret);
			kfree(data);
			return ERROR_IIC;
		}
...
static s32 _do_i2c_write(struct i2c_msg *msg, u16 addr, u8 *buffer, s32 len)
{
	s32 ret = -1;
	s32 pos = 0;
	s32 data_length = len;
	...
	while (pos != data_length) {
		if ((data_length - pos) > (IIC_MAX_TRANSFER_SIZE - FT5X0X_ADDR_LENGTH))
			transfer_length = IIC_MAX_TRANSFER_SIZE - FT5X0X_ADDR_LENGTH;
		else
			transfer_length = data_length - pos;

		msg->buf[0] = (address >> 8) & 0xFF;
		msg->buf[1] = address & 0xFF;
		msg->len = transfer_length + FT5X0X_ADDR_LENGTH;
		memcpy(&msg->buf[FT5X0X_ADDR_LENGTH], &buffer[pos], transfer_length);
...
static s32 i2c_write_mtk(u16 addr, u8 *buffer, s32 len)
{
	...
	struct i2c_msg msg = {
		.flags = 0,
		.addr = i2c_client->addr,  /*remain*/
	};

	TPD_DEBUG("%s  %d addr =0x%x\n" , __func__ , __LINE__ ,i2c_client->addr);
...
{
	int ret;
	u8 addr_buf[FT5X0X_ADDR_LENGTH] = { (addr >> 8) & 0xFF, addr & 0xFF };
...
static s32 i2c_smbus_write_i2c_block_data_(const struct i2c_client *client, u8 command,
				   u8 length, const u8 *values)
{
	return i2c_write_mtk(command , (u8*)values, length);
}

static s32 i2c_smbus_read_i2c_block_data_(const struct i2c_client *client, u8 command,
				  u8 length, u8 *values)
{
	return i2c_read_mtk(command , values , length);
}
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
	...
	i2c_smbus_read_i2c_block_data_(i2c_client, 0x00, 8, &(data[0]));
	i2c_smbus_read_i2c_block_data_(i2c_client, 0x08, 8, &(data[8]));
	i2c_smbus_read_i2c_block_data_(i2c_client, 0x10, 8, &(data[16]));
	i2c_smbus_read_i2c_block_data_(i2c_client, 0x18, 8, &(data[24]));
	i2c_smbus_read_i2c_block_data_(i2c_client, 0xa6, 1, &(data[32]));
	i2c_smbus_read_i2c_block_data_(i2c_client, 0x88, 1, &report_rate);
	...
	if (report_rate < 8) {
		report_rate = 0x8;
		if ((i2c_smbus_write_i2c_block_data_(i2c_client, 0x88, 1, &report_rate)) < 0)
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		TPD_DEBUG("%s  %d touch_irq=%d \n" , __func__ , __LINE__ , touch_irq);
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	msleep(50);
#else
	FT5X0X_GPIO_OUTPUT(FT5X0X_RST_PORT,0);
	msleep(20);
	FT5X0X_GPIO_OUTPUT(FT5X0X_RST_PORT,1);	
	msleep(50);	
#endif
...
#else
	FT5X0X_GPIO_AS_INT(FT5X0X_INT_PORT);
#endif
...
reset_proc:
	...
	do
	{
		...
		FT5X0X_GPIO_OUTPUT(FT5X0X_RST_PORT,0);
		msleep(10);			
		if(i2c_smbus_read_i2c_block_data_(i2c_client, 0x00, 1, &data)<0)
		{
...

	{
		...
		i2c_smbus_read_i2c_block_data_(client, 0xA6, 1, &ver);

		TPD_DMESG(TPD_DEVICE " i2c_smbus_read_i2c_block_data_ version : %d\n", ver);
	}
...
static s8 ftp_enter_doze(struct i2c_client *client)
{
	...
	ret = i2c_smbus_write_i2c_block_data_(i2c_client, FT_GESTRUE_MODE_SWITCH_REG, 1, &gestrue_on);
	if (ret < 0) {
	...
	for (i = 0; i < 10; i++) {
		i2c_smbus_read_i2c_block_data_(i2c_client, FT_GESTRUE_MODE_SWITCH_REG, 1, &gestrue_data);
		...
		msleep(20);
		i2c_smbus_write_i2c_block_data_(i2c_client, FT_GESTRUE_MODE_SWITCH_REG, 1, &gestrue_on);
...
static void tpd_resume(struct device *h)
{
	...
	i2c_smbus_write_i2c_block_data_(i2c_client, 0xd0, 1, &data);
#else
	enable_irq(touch_irq);
#endif
...
static void tpd_suspend(struct device *h)
{
	...
	/* { */
		...
		/* TP enter sleep mode */
		/* i2c_smbus_write_i2c_block_data_(i2c_client_point, 0xA5, 1, &gestrue_cmd); */
		/* if (ret < 0) */
		/* { */
...
#else
	disable_irq(touch_irq);
	i2c_smbus_write_i2c_block_data_(i2c_client, 0xA5, 1, &data);  /* TP enter sleep mode */
