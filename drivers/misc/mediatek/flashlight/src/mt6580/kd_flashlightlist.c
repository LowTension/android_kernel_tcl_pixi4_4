...
#ifdef CONFIG_OF
struct pinctrl *strobepinctrl1;
struct pinctrl_state *strobe_pins_default;
struct pinctrl_state *enable_output0, *enable_output1, *torch_output0, *torch_output1,*enablef_output0,*enablef_output1;
...
/* ======================================================================== */
static DEFINE_MUTEX(strobe_set_gpio_mutex);

void strobe_gpio_output(int pin, int level)
{
	mutex_lock(&strobe_set_gpio_mutex);
	printk("[strobe]strobe_gpio_output pin = %d, level = %d\n", pin, level);
	if (pin == 1) {
		if (level)
			pinctrl_select_state(strobepinctrl1, enable_output1);
		else
			pinctrl_select_state(strobepinctrl1, enable_output0);
	} else if (pin == 0) {
		if (level)
			pinctrl_select_state(strobepinctrl1, torch_output1);
		else
			pinctrl_select_state(strobepinctrl1, torch_output0);
	}
	else if (pin == 2) {
		if (level)
			pinctrl_select_state(strobepinctrl1, enablef_output1);
		else
			pinctrl_select_state(strobepinctrl1, enablef_output0);
	}
	mutex_unlock(&strobe_set_gpio_mutex);
}
int strobe_get_gpio_info(struct platform_device *pdev)
{
	int ret;
	printk("[strobe %d] mt_strobe_pinctrl+++++++++++++++++\n", pdev->id);
	strobepinctrl1 = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(strobepinctrl1)) {
		ret = PTR_ERR(strobepinctrl1);
		dev_err(&pdev->dev, "fwq Cannot find strobepinctrl1!\n");
		return ret;
	}
	strobe_pins_default = pinctrl_lookup_state(strobepinctrl1, "default");
	if (IS_ERR(strobe_pins_default)) {
		ret = PTR_ERR(strobe_pins_default);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl default %d!\n", ret);
	}

	enable_output0 = pinctrl_lookup_state(strobepinctrl1, "state_enable_output0");
	if (IS_ERR(enable_output0)) {
		ret = PTR_ERR(enable_output0);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_enable_output0!\n");
		return ret;
	}
	enable_output1 = pinctrl_lookup_state(strobepinctrl1, "state_enable_output1");
	if (IS_ERR(enable_output1)) {
		ret = PTR_ERR(enable_output1);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_enable_output1!\n");
		return ret;
	}
	torch_output0 = pinctrl_lookup_state(strobepinctrl1, "state_torch_output0");
	if (IS_ERR(torch_output0)) {
		ret = PTR_ERR(torch_output0);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_torch_output0!\n");
		return ret;
	}
	torch_output1 = pinctrl_lookup_state(strobepinctrl1, "state_torch_output1");
	if (IS_ERR(torch_output1)) {
		ret = PTR_ERR(torch_output1);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_torch_output1!\n");
		return ret;
	}
	enablef_output0 = pinctrl_lookup_state(strobepinctrl1, "state_enablef_output0");
	if (IS_ERR(enablef_output0)) {
		ret = PTR_ERR(enablef_output0);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_enablef_output0!\n");
		return ret;
	}
	enablef_output1 = pinctrl_lookup_state(strobepinctrl1, "state_enablef_output1");
	if (IS_ERR(enablef_output1)) {
		ret = PTR_ERR(enablef_output1);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_enablef_output1!\n");
		return ret;
	}
	printk("[strobe%d] mt_strobe_pinctrl----------\n", pdev->id);
	return 0;
