...
//BEGIN: add by fangjie for defect 1999292
#define FORCE_SET_KERNEL_DSI_TIMING

#ifdef FORCE_SET_KERNEL_DSI_TIMING
#include "ddp_reg.h"
#define OFFSET_DSI_VSA_NL  0x0020
#define OFFSET_DSI_VBP_NL  0x0024
#define OFFSET_DSI_VFP_NL  0x0028
#define OFFSET_DSI_VACT_NL 0x002C
#endif
//END: add by fangjie for defect 1999292
...
//BEGIN: add by fangjie for defect 1999292
/*
	-- force setting the Verital porch value when LCM resume( lcm_resume will call lcm_init()).
    Because in MTK default LCM initial process，LK phase completed the LCD IC initialization and AP DSI interface initialization (call lcm_init() and lcm_get_params), 
	then Kernel phase no longer do this intial action. 	But when the LCM resume, the lcm_init() will be called. 
    So, Under the FOTA scenario, Boot.img(include kernel) has been upgraded but not upgrade lk.bin, 
	will cause the software after FOTA may dismatch between DSI porch(VBP/VFP/VSA..) and LCD IC register.
	And cause the white line after LCM resume. 
*/
#ifdef FORCE_SET_KERNEL_DSI_TIMING
static void force_set_kernel_dsi_timing(disp_lcm_handle *plcm)
{	
	static int isFirstResume = 0;	
	LCM_PARAMS *p = NULL;
	p = plcm->params;
	
	if (0 == isFirstResume){
		printk("fangjie : lcm is first Resume\n");
		DISP_REG_SET(NULL, DISPSYS_DSI0_BASE + OFFSET_DSI_VSA_NL, p->dsi.vertical_sync_active);
		DISP_REG_SET(NULL, DISPSYS_DSI0_BASE + OFFSET_DSI_VBP_NL, p->dsi.vertical_backporch);
		DISP_REG_SET(NULL, DISPSYS_DSI0_BASE + OFFSET_DSI_VFP_NL, p->dsi.vertical_frontporch);
		DISP_REG_SET(NULL, DISPSYS_DSI0_BASE + OFFSET_DSI_VACT_NL, p->dsi.vertical_active_line);
		isFirstResume = 1;	
	}
}
#endif
...
int disp_lcm_resume(disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->resume) {
			//BEGIN: add by fangjie for defect 1999292
			#ifdef FORCE_SET_KERNEL_DSI_TIMING
			    force_set_kernel_dsi_timing(plcm); 
			#endif
			//END: add by fangjie for defect 1999292
			lcm_drv->resume();
