...
static int read_otp_id(struct otp_struct *otp_ptr)
{
	int otp_flag, addr, i;
	int temp1;
	//set 0x5002[3] to ¡°0¡±
	...
	write_cmos_sensor(0x5002,(0x08)|(temp1&(~0x08)));
       LOG_INF("[ov5670 otp]otp_flag = %x ,module_integrator_id = %x, lens_id = %x ,vcm_id=%x\n",otp_flag, (*otp_ptr).module_integrator_id, (*otp_ptr).lens_id,(*otp_ptr).vcm_id );
	  if(((*otp_ptr).module_integrator_id==0x44)&&((*otp_ptr).lens_id==0xb1)&&((*otp_ptr).vcm_id ==0x00))
	  (*otp_ptr).flag |= 0x40;
	  
	return (*otp_ptr).flag;
}
static int read_otp_cail(struct otp_struct *otp_ptr)
{
	int otp_flag, addr, i;
	int temp=0 ,temp1=0;
	//set 0x5002[3] to ¡°0¡±
	...
	LOG_INF("[ov5670 otp]rg_h = %x, rg_l = %x, bg_h = %x, bg_l = %x\n", read_cmos_sensor(addr + 1), ((temp>>6) & 0x03), read_cmos_sensor( addr + 2), ((temp>>4) & 0x03));
	LOG_INF("[ov5670 otp]rg = %x, bg = %x (*otp_ptr).flag =%x\n", (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio,(*otp_ptr).flag);
...
static void apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40) {
		...
		LOG_INF("[ov5670 otp]g_rg = %x, g_bg = %x\n", g_rg, g_bg);
		//calculate G gain
		R_gain = (g_rg * 1000) / rg;
		B_gain = (g_bg * 1000) / bg;
		G_gain = 1000;
		...
		LOG_INF("[ov5670 otp]rg=%x, bg=%x, R_gain=%x, B_gain=%x, G_gain=%x\n", rg,bg,R_gain,B_gain,G_gain);
		...
		do 
		{
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) 
			{
				...
				#ifdef OV5670_OTP_CALI
				if( read_otp_id(&ov5670_otp_data)==0xc0)
				return ERROR_NONE;
				#endif
			}
...
static kal_uint32 open(void)
{
	...
	sensor_init();
#ifdef OV5670_OTP_CALI
	if(read_otp_cail(&ov5670_otp_data)==0xe0)
	{
	   apply_otp(&ov5670_otp_data);
	}
	else 
	return ERROR_DRIVER_INIT_FAIL;
	ov5670_otp_data.flag=0xc0;
#endif
	spin_lock(&imgsensor_drv_lock);
